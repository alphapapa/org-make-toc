...
that aren't intended to be rendered by GitHub, so it must be manually enabled in each document as appropriate.  For many cases, an
adequate workaround can be had by marking the heading that contains the table of contents with a =:noexport:= modifier.
